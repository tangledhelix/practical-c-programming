...
int main( void ) {
	...
	if ( end_year - start_year > 1 ) {
		...
		result = get_delta( start_month, start_day, end_month, end_day, start_year );
		total += result;

#ifdef DEBUG
		printf( "DEBUG:main(): got delta %d for %d/%d - %d/%d in %d\n",
				result, start_month, start_day, end_month, end_day, start_year );
#endif
...
#ifdef DEBUG
		printf( "DEBUG:get_delta(): got %d delta for month %d", result, start_month );
#endif
...
#ifdef DEBUG
		printf( "DEBUG:get_delta(): got %d delta for month %d", end_day, end_month );
#endif
